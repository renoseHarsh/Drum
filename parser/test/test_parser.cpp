...
TEST_CASE("Missing Command") {
  ...
  const auto result = drum::parser::parse_arguments(1, argv);
...
TEST_CASE("Unknown command") {
  ...
  const auto result = drum::parser::parse_arguments(2, argv);
  REQUIRE_FALSE(result.has_value());
...
auto check_new_args = [](const auto &result,
                         drum::commands::NewArgs::PackageType type,
                         std::string_view name) {
  REQUIRE(result.has_value());

  const auto *new_args = std::get_if<drum::commands::NewArgs>(&result.value());
...
TEST_CASE("New command creates executable package") {
  ...
  const auto result = drum::parser::parse_arguments(3, argv);
  check_new_args(result, drum::commands::NewArgs::PackageType::executable,
                 "pkg");
}
...
TEST_CASE("New command creates library package") {
  ...
  const auto result = drum::parser::parse_arguments(4, argv);
  check_new_args(result, drum::commands::NewArgs::PackageType::library, "pkg");
}
